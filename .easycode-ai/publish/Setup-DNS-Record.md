...
|---|---|---|---|---|
| A | @ | 216.150.1.1 | Auto/300s | Required: Make vscode.com direct to the site |
| CNAME | www | c254ca7ed118f125.vercel-dns-016.com. | Auto/300s | Optional: Make www.vscode.com also accessible |
...
3. Add records one by one according to the table:
   ...
   - Host Record(Name):
     - Top-level Domain(such as vscode.com) use "@"(some suppliers leave empty to represent @).
     - Subdomain(such as vscode.com) use prefix "@".
...
- **Subdomain Access**: If the subdomain (such as blog.vscode.com) is accessed, usually only the CNAME record in the table needs to be added.
